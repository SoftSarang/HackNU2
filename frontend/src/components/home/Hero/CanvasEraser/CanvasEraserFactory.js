...
const factory = () => {
  ...
  const _onMouseMove = event => {
    ...
    const { enabled, scaleRatio, touchX, touchY } = _data;
    // compute latest bounding rect so we stay aligned with transforms/scroll
    const rect = _canvas.getBoundingClientRect();
    const posX = rect.left;
    const posY = rect.top;
    const currentX = (event.clientX - posX) * scaleRatio;
    const currentY = (event.clientY - posY) * scaleRatio;
  ...
  const _onMouseDown = event => {
    ...
    const { enabled, scaleRatio } = _data;
    const rect = _canvas.getBoundingClientRect();
    const posX = rect.left;
    const posY = rect.top;
    const currentX = (event.clientX - posX) * scaleRatio;
  ...
  const _onMouseClick = event => {
    ...
    _canvas.addEventListener('mousemove', _onMouseMove);
    const _onMouseUpClick = () => {
      _canvas.removeEventListener('mousemove', _onMouseMove);
      document.removeEventListener('mouseup', _onMouseUpClick);
      _data.touchDown = false;
    };
    document.addEventListener('mouseup', _onMouseUpClick);
  };
  ...
  const realHeight = getElementComputedStyled(_canvas, 'height');
    ...
    // use bounding rect so posX/posY are viewport-based (works with fixed/absolute)
    const rect = _canvas.getBoundingClientRect();
